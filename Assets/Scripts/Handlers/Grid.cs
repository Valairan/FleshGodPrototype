...
public class Grid : MonoBehaviour
{
	...
	public List<Vector3> generateRandomRow()
	{
		...
		{
			positions.Add(new Vector3(i, -6 + Random.Range(-1f, 1f), index));
		}
	...
	public List<Vector3> generateRandomColumn()
	{
		...
		{
			positions.Add(new Vector3(index, -6 + Random.Range(-1f, 1f), i));
		}
		...
		switch (index)
		{
			case 0:
				for (int j = 0; j <= horizontalSize / 2; j++)
				{
					...
					{
						positions.Add(new Vector3(j, -6 + Random.Range(-1f, 1f), i));
					}
				}
				break;
			case 1:
